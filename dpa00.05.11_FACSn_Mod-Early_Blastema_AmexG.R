...
BL_dpa00.05.11 = merge(dpa00_FACSn_GER006, y = c(dpa00_FACSn_GER007, dpa05_FACSn_GER043, dpa05_FACSn_GER045, dpa11_FACSn_GER047), add.cell.ids = c("GER006", "GER007", "GER043", "GER045", "GER047"), project = "dpa00.05.11")

mito.genes <- c("ND2","ND1","ND3","ND4","ND4L","ND5","ND6")
mito.contig <- intersect(c(rownames(anno)[anno$V2 %in% mito.genes ] , rownames(anno)[anno$V2 %in% anno$V2[grep("^COX",anno$V2)]] ) , rownames(BL_dpa00.05.11))


BL_dpa00.05.11[["percent.mt"]] <- PercentageFeatureSet(BL_dpa00.05.11, features = mito.contig)
...
BL_dpa00.05.11 = NormalizeData(BL_dpa00.05.11)

plot1 <- FeatureScatter(BL_dpa00.05.11, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(BL_dpa00.05.11, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")


pdf("./BL_dpa00.05.11_QC_PreFilter.pdf",width=10,height=5)
VlnPlot(BL_dpa00.05.11, features = c("nCount_RNA","nFeature_RNA","percent.mt"),pt.size = -1)
CombinePlots(plots = list(plot1, plot2))
dev.off()

BL_dpa00.05.11 <- subset(BL_dpa00.05.11, subset = nCount_RNA < 20000  & nCount_RNA > 500  & percent.mt < 10 )

plot1 <- FeatureScatter(BL_dpa00.05.11, feature1 = "nCount_RNA", feature2 = "percent.mt")
plot2 <- FeatureScatter(BL_dpa00.05.11, feature1 = "nCount_RNA", feature2 = "nFeature_RNA")

pdf("./BL_dpa00.05.11_QC_PostFilter.pdf",width=10,height=5)
VlnPlot(BL_dpa00.05.11, features = c("nCount_RNA","nFeature_RNA","percent.mt"),pt.size = -1)
CombinePlots(plots = list(plot1, plot2))
dev.off()
...
g2m.contig <- intersect(rownames(anno)[anno$V2 %in% g2m.genes ] , rownames(BL_dpa00.05.11) )

s.genes <- cc.genes$s.genes
s.contig <- intersect(rownames(anno)[anno$V2 %in% s.genes ] , rownames(BL_dpa00.05.11) )

#cell cycle scoring
BL_dpa00.05.11 <- CellCycleScoring(BL_dpa00.05.11, s.features = s.contig, g2m.features = g2m.contig, set.ident = TRUE)
...
all.genes <- rownames(BL_dpa00.05.11)

BL_dpa00.05.11 = ScaleData(  BL_dpa00.05.11, features = all.genes ,vars.to.regress = c("nCount_RNA","nFeature_RNA","percent.mt","S.Score", "G2M.Score","eGFP","mCherry"))
...
BL_dpa00.05.11 = RunPCA(BL_dpa00.05.11,features = all.genes,npcs = 100)
...
pdf("BL_dpa00.05.11_PCAheatmaps.pdf",width=30,height=20)
DimHeatmap(BL_dpa00.05.11, dims = 1:16, cells = 1000, balanced = TRUE, ncol = 4,  fast = F) 
DimHeatmap(BL_dpa00.05.11, dims = 17:32, cells = 1000, balanced = TRUE, ncol = 4,  fast = F) 
DimHeatmap(BL_dpa00.05.11, dims = 33:48, cells = 1000, balanced = TRUE,  ncol = 4, fast = F) 
DimHeatmap(BL_dpa00.05.11, dims = 49:64, cells = 1000, balanced = TRUE,  ncol = 4, fast = F) 
DimHeatmap(BL_dpa00.05.11, dims = 65:80, cells = 1000, balanced = TRUE,  ncol = 4, fast = F) 
DimHeatmap(BL_dpa00.05.11, dims = 81:96, cells = 1000, balanced = TRUE,  ncol = 4, fast = F) 
dev.off()

#elbow plot
pdf("BL_dpa00.05.11_PCelbow.pdf",width=20,height=10)
ElbowPlot(BL_dpa00.05.11, ndims = 100)
dev.off()

#Cluster cells
BL_dpa00.05.11<- FindNeighbors(BL_dpa00.05.11, dims = 1:30)
BL_dpa00.05.11 <- FindClusters(BL_dpa00.05.11, resolution = 0.5)

#run UMAP
#min.dist deafult is 0.3. Can go down to 0.001. Play around to modify clustering
BL_dpa00.05.11 = RunUMAP(BL_dpa00.05.11,dims = 1:30)

#run tSNE
BL_dpa00.05.11 = RunTSNE(BL_dpa00.05.11,dims = 1:30)
...
pdf("BL_dpa00.05.11_Embedding_PC30_res0.5.pdf",width=10,height=10)
DimPlot(object = BL_dpa00.05.11, reduction = 'umap', pt.size = 2)
DimPlot(object = BL_dpa00.05.11, reduction = 'umap', pt.size = 2,group.by = "orig.ident")
DimPlot(object = BL_dpa00.05.11, reduction = 'tsne', pt.size = 2)
dev.off()

#some QC features

pdf("BL_dpa00.05.11_feature.pdf",width=8,height=10)
FeaturePlot(BL_dpa00.05.11, reduction = 'umap', pt.size = 0.5, features = c("nFeature_RNA","nCount_RNA","percent.mt","mCherry","eGFP"),order = T, cols = c(brewer.pal(9,"Greys")[9:2],brewer.pal(9,"Reds")[2:9]))
FeaturePlot(BL_dpa00.05.11, reduction = 'tsne', pt.size = 0.5, features = c("nFeature_RNA","nCount_RNA","percent.mt","mCherry","eGFP"),order = T, cols = c(brewer.pal(9,"Greys")[9:2],brewer.pal(9,"Reds")[2:9]))
dev.off()
...
markers <- FindAllMarkers(BL_dpa00.05.11, only.pos = TRUE,  logfc.threshold = 0.3)
...
write.csv(markers.anno,"BL_dpa00.05.11_allMarker.csv")

saveRDS(BL_dpa00.05.11, file = "BL_dpa00.05.11_SeuratObj.RDS")
...
gg_Fig <- FeaturePlot(BL_dpa00.05.11, pt.size = 0.5, features = gene_ids,order = T, cols = brewer.pal(9,"Greys")[3:9], repel=T)
gg_Fig <- lapply( 1:length(gene_ids), function(x) { gg_Fig[[x]] + labs(title=gene_names[x]) })

pdf("BL_dpa00.05.11_UMAP_feature_ClustMarker.pdf",width=14,height=10)
CombinePlots( gg_Fig )
dev.off()
